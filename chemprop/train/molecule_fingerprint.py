...
def molecule_fingerprint(
    args: FingerprintArgs, smiles: List[List[str]] = None
    ...
    if (
        args.fingerprint_type == "MPN"
    ):  # only need to supply input features if using FFN latent representation and if model calls for them.
    ...
    if args.fingerprint_type == "MPN":
        ...
        ):  # special case when we have 'descriptor' extra dimensions need to be added
            total_fp_size = (
                args.hidden_size + test_data.atom_descriptors_size()
    ...
    for index, checkpoint_path in enumerate(
        ...
        (
            ...
            bond_feature_scaler,
        ) = load_scalers(args.checkpoint_paths[index])

        # Normalize features
        if (
            args.features_scaling
            or train_args.atom_descriptor_scaling
    ...
    for full_index, datapoint in enumerate(full_data):
        ...
        preds = (
            all_fingerprints[valid_index].reshape(
                (len(args.checkpoint_paths) * total_fp_size)
            )
            if valid_index is not None
            else ["Invalid SMILES"] * len(args.checkpoint_paths) * total_fp_size
        )
    ...
    with open(args.preds_path, "w") as f:
        writer = csv.DictWriter(
            f,
            fieldnames=args.smiles_columns + fingerprint_columns,
            extrasaction="ignore",
        )
        writer.writeheader()
