    ...
    * :code:`.pkl` / :code:`.pckl` / :code:`.pickle` containing a sparse numpy array
    ...
    if extension == ".npz":
        ...
        with open(path, "rb") as f:
            features = np.array(
                [np.squeeze(np.array(feat.todense())) for feat in pickle.load(f)]
            )
    else:
    ...
    if extension == ".npz":
        ...
        if features_df.iloc[0, 0].ndim == 1:
            features = features_df.apply(
                lambda x: np.stack(x.tolist(), axis=1), axis=1
            ).tolist()
        elif features_df.iloc[0, 0].ndim == 2:
            features = features_df.apply(
                lambda x: np.concatenate(x.tolist(), axis=1), axis=1
            ).tolist()
        else:
        ...
        features_df = (
            PandasTools.LoadSDF(path).drop(["ID", "ROMol"], axis=1).set_index("SMILES")
        )
        ...
        features_df = features_df.iloc[
            :,
            features_df.iloc[0, :]
            .apply(lambda x: isinstance(x, str) and "," in x)
            .to_list(),
        ]
