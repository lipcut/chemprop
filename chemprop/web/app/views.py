...
def name_already_exists_message(
    thing_being_named: str, original_name: str, new_name: str
) -> str:
    """
    ...
    """
    return (
        f'{thing_being_named} "{original_name} already exists. '
...
def train():
    ...
    data_name, epochs, ensemble_size, checkpoint_name = (
        ...
        int(request.form["epochs"]),
        int(request.form["ensembleSize"]),
    ...
    ckpt_id, ckpt_name = db.insert_ckpt(
        checkpoint_name,
        current_user,
        args.dataset_type,
        args.epochs,
    ...
    with TemporaryDirectory() as temp_dir:
        ...
        logger = create_logger(
            name=TRAIN_LOGGER_NAME, save_dir=args.save_dir, quiet=args.quiet
        )
        ...
        if checkpoint_name != ckpt_name:
            warnings.append(
                name_already_exists_message("Checkpoint", checkpoint_name, ckpt_name)
            )
...
def predict():
    ...
    if request.form["textSmiles"] != "":
        ...
        smiles = (
            [possible_smiles] if Chem.MolFromSmiles(possible_smiles) is not None else []
        )
    ...
    arguments = [
        ...
        "None",
        "--preds_path",
        os.path.join(app.config["TEMP_FOLDER"], app.config["PREDICTIONS_FILENAME"]),
        "--checkpoint_paths",
        *model_paths,
    ]
...
def upload_data(return_page: str):
    ...
    with NamedTemporaryFile() as temp_file:
        ...
        if len(dataset_errors) > 0:
            ...
            if dataset_name != new_dataset_name:
                warnings.append(
                    name_already_exists_message("Data", dataset_name, new_dataset_name)
                )
    ...
    return redirect(
        url_for(return_page, data_upload_warnings=warnings, data_upload_errors=errors)
    )
...
def upload_checkpoint(return_page: str):
    ...
    if ckpt_ext.endswith(".pt"):
        ...
        for root, _, fnames in os.walk(ckpt_dir):
            ckpt_paths += [
                os.path.join(root, fname) for fname in fnames if fname.endswith(".pt")
            ]
    ...
    if len(ckpt_paths) > 0:
        ...
        ckpt_id, new_ckpt_name = db.insert_ckpt(
            ckpt_name,
            current_user,
        ...
        for ckpt_path in ckpt_paths:
            ...
            if ckpt_name != new_ckpt_name:
                warnings.append(
                    name_already_exists_message("Checkpoint", ckpt_name, new_ckpt_name)
                )
