...
def set_root_folder(
    app: Flask, root_folder: str = None, create_folders: bool = True
) -> None:
    """
    ...
    if root_folder is not None:
        ...
        app.config["DATA_FOLDER"] = os.path.join(
            app.config["ROOT_FOLDER"], "app/web_data"
        )
        app.config["CHECKPOINT_FOLDER"] = os.path.join(
            app.config["ROOT_FOLDER"], "app/web_checkpoints"
        )
        app.config["TEMP_FOLDER"] = os.path.join(app.config["ROOT_FOLDER"], "app/temp")
        app.config["DB_PATH"] = os.path.join(
            app.config["ROOT_FOLDER"], app.config["DB_FILENAME"]
        )
    ...
    if create_folders:
        if not os.access(os.path.dirname(app.config["ROOT_FOLDER"]), os.W_OK):
            raise ValueError(
                f'You do not have write permissions on the root_folder: {app.config["ROOT_FOLDER"]}\n'
