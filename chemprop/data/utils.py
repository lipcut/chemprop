...
def get_smiles(
    ...
    if smiles_columns is not None and not header:
        raise ValueError(
            "If smiles_column is provided, the CSV file must have a header."
        )
...
def get_data(
    ...
    if args is not None:
        ...
        smiles_columns = (
            smiles_columns if smiles_columns is not None else args.smiles_columns
        )
        target_columns = (
            target_columns if target_columns is not None else args.target_columns
        )
        ignore_columns = (
            ignore_columns if ignore_columns is not None else args.ignore_columns
        )
        features_path = (
        ...
        features_generator = (
            ...
            else args.features_generator
        )
        phase_features_path = (
            phase_features_path
            if phase_features_path is not None
            else args.phase_features_path
        )
        ...
        loss_function = (
            loss_function if loss_function is not None else args.loss_function
        )
    ...
    if loss_function == "bounded_mse":
        gt_targets, lt_targets = get_inequality_targets(
        ...
        data = MoleculeDataset(
            [
                MoleculeDatapoint(
                    ...
                    phase_features=all_phase_features[i]
                    if phase_features is not None
                    else None,
...
def split_data(
    ...
    if not (len(sizes) == 3 and np.isclose(sum(sizes), 1)):
        raise ValueError(
            f"Split sizes do not sum to 1. Received train/val/test splits: {sizes}"
        )
...
def get_class_sizes(
    data: MoleculeDataset, proportion: bool = True
) -> List[List[float]]:
    """
...
def validate_dataset_type(data: MoleculeDataset, dataset_type: str) -> None:
    ...
    if dataset_type == "classification" and not (
        target_set <= classification_target_set
    ):
        raise ValueError(
