...
def normalize_spectra(
    ...
    phase_features: List[List[float]] = None,
    phase_mask: List[List[float]] = None,
    batch_size: int = 50,
    excluded_sub_value: float = None,
    ...
    for i in trange(0, num_iters, iter_step):
        ...
        if phase_exclusion:
            ...
            batch_mask = ~(
                ~batch_mask + ~batch_phase_mask
            )  # mask shows True only if both components true
...
def roundrobin_sid(spectra: np.ndarray, threshold: float = None) -> List[float]:
    ...
    for i in range(len(spectra)):
        ...
        for j in range(ensemble_size - 1):
            ensemble_tail = np.concatenate(
                (ensemble_tail, spectrum[:, j + 1 :]), axis=1
            )
            ensemble_head = np.concatenate(
                (ensemble_head, spectrum[:, : -j - 1]), axis=1
            )
        loss = ensemble_head * np.log(
            ensemble_head / ensemble_tail
        ) + ensemble_tail * np.log(ensemble_tail / ensemble_head)
        loss[nan_mask, :] = 0
    ...
    with open(path, "r") as rf:
        ...
        for line in r:
            if any([x not in ["0", "1"] for x in line[1:]]):
                raise ValueError(
                    "Phase mask must contain only 0s and 1s, with 0s indicating exclusion regions."
                )
