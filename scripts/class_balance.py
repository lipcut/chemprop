...
class Args(Tap):
    ...
    smiles_columns: List[
        str
    ] = None  # Name of the columns containing SMILES strings. By default, uses the first column.
    target_columns: List[
        str
    ] = None  # Name of the columns containing target values. By default, uses all columns except the SMILES column.
    split_type: Literal[
        "random", "scaffold"
    ] = "scaffold"  # Split type, either "random" or "scaffold"
...
def class_balance(args: Args):
    ...
    for split_name in ["train", "val", "test"]:
        ...
        mean_class_sizes, std_class_sizes = np.mean(
            all_class_sizes[split_name], axis=0
        ), np.std(all_class_sizes[split_name], axis=0)
