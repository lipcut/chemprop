...
def aggregate_results_by_dataset(dataset: str, ckpt_dir: str, split_type: str):
    ...
    for experiment in EXPERIMENTS:
        ...
        for root, _, files in os.walk(exp_dir):
            paths += [
                os.path.join(root, fname) for fname in files if fname == "verbose.log"
            ]
    ...
    for i in range(max_num_folds):
        for experiment in EXPERIMENTS:
            print(
                results[experiment][i] if i < len(results[experiment]) else "", end="\t"
            )
        print()
...
if __name__ == "__main__":
    ...
    aggregate_results_by_dataset(
        dataset=args.dataset, ckpt_dir=args.ckpt_dir, split_type=args.split_type
    )
