...
class Args(Tap):
    ...
    test_folds_to_test: int = (
        None  # Number of cross validation folds to test as test folds
    )
...
def create_crossval_indices(args: Args):
    ...
    for i in folds[: args.test_folds_to_test]:
        with open(os.path.join(args.save_dir, f"{i}_opt.pkl"), "wb") as valf, open(
            os.path.join(args.save_dir, f"{i}_test.pkl"), "wb"
        ) as testf:
            index_sets = []
            test_index_sets = []
            ...
            for val_index in index_folds[: args.val_folds_per_test]:
                train, val, test = (
                    [index for index in index_folds if index != val_index],
                    [val_index],
                    [i],
                )  # test set = val set during cv for now
        ...
        for j in range(len(index_sets)):
            with open(
                os.path.join(args.save_dir, "mayr", f"{i}_{j}_opt.pkl"), "wb"
            ) as valf, open(
                os.path.join(args.save_dir, "mayr", f"{i}_{j}_test.pkl"), "wb"
            ) as testf:
