...
def lsc_to_our_format(lsc_dir: str, ckpt_dir: str, save_dir: str):
    ...
    for dataset in DATASETS:
        ...
        for fold in range(10):
            lsc_preds_path = os.path.join(
                lsc_dir,
                dataset,
                "test",
                f"fold_{fold}",
                "semi",
                "o0003.evalPredict.hdf5",
            )
            ckpt_targets_path = os.path.join(
                ckpt_dir, dataset, "scaffold", str(fold), "targets.npy"
            )

            if not (
                os.path.exists(lsc_preds_path) and os.path.exists(ckpt_targets_path)
            ):
                continue
...
if __name__ == "__main__":
    ...
    lsc_to_our_format(
        lsc_dir=args.lsc_dir, ckpt_dir=args.ckpt_dir, save_dir=args.save_dir
    )
