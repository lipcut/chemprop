...
class Args(Tap):
    ...
    nobs1: List[
        int
    ]  # Number of observations each mean/std is constructed from in 1st model
    nobs2: List[
        int
    ]  # Number of observations each mean/std is constructed from in 2nd model


def welchs(
    mean1: List[float],  # mean performance across folds for each dataset (model 1)
    std1: List[
        float
    ],  # standard deviation performance across folds for each dataset (model 1)
    ...
    std2: List[
        float
    ],  # standard deviation performance across folds for each dataset (model 2)
    nobs2: List[int],
):  # number of CV folds for each dataset (model 2)
    ...
    assert (
        len(mean1) == len(std1) == len(nobs1) == len(mean2) == len(std2) == len(nobs2)
    )
    ...
    pvalues = [
        stats.ttest_ind_from_stats(
            mean1=m1, std1=s1, nobs1=n1, mean2=m2, std2=s2, nobs2=n2, equal_var=False
        ).pvalue
        / 2
