...
class Args(Tap):
    ...
    similarity_measure: Literal[
        "scaffold", "morgan"
    ]  # Similarity measure to use to compare the two datasets
...
def scaffold_similarity(smiles_1: List[str], smiles_2: List[str]):
    ...
    smiles_in_1_with_scaffold_in_2 = {
        smiles for smiles in smiles_1 if smiles_to_scaffold[smiles] in scaffolds_2
    }
    smiles_in_2_with_scaffold_in_1 = {
        smiles for smiles in smiles_2 if smiles_to_scaffold[smiles] in scaffolds_1
    }
    ...
    print(
        " | ".join(
            [f"{i}% = {int(np.percentile(sizes_1, i)):,}" for i in range(0, 101, 10)]
        )
    )
    print()
    print(
...
def morgan_similarity(
    smiles_1: List[str], smiles_2: List[str], radius: int, sample_rate: float
):
    """
    ...
    for smile_1, smile_2 in tqdm(
        product(sample_smiles_1, sample_smiles_2), total=sample_num_pairs
    ):
        mol_1, mol_2 = Chem.MolFromSmiles(smile_1), Chem.MolFromSmiles(smile_2)
        fp_1, fp_2 = AllChem.GetMorganFingerprint(
            mol_1, radius
        ), AllChem.GetMorganFingerprint(mol_2, radius)
...
if __name__ == "__main__":
    ...
    smiles_1 = get_smiles(
        path=args.data_path_1, smiles_columns=args.smiles_column_1, flatten=True
    )
    smiles_2 = get_smiles(
        path=args.data_path_2, smiles_columns=args.smiles_column_2, flatten=True
    )
