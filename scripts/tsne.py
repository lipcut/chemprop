...
class Args(Tap):
    ...
    colors: List[str] = [
        ...
        "purple",
        "blue",
    ]  # Colors of the points associated with each dataset
    sizes: List[float] = [
        1,
        1,
        1,
        1,
        1,
    ]  # Sizes of the points associated with each molecule
    scale: int = 1  # Scale of figure
    plot_molecules: bool = (
        False  # Whether to plot images of molecules instead of points
    )
...
def compare_datasets_tsne(args: Args):
    if len(args.smiles_paths) > len(args.colors) or len(args.smiles_paths) > len(
        args.sizes
    ):
    ...
    for smiles_path in args.smiles_paths:
        ...
        new_smiles = get_smiles(
            path=smiles_path, smiles_columns=args.smiles_column, flatten=True
        )
    ...
    plt.title(
        "t-SNE using Morgan fingerprint with Jaccard similarity", fontsize=2 * fontsize
    )
    ax = fig.gca()
    handles = []
            ...
            if args.plot_molecules:
                ...
                for smile, (x, y) in zip(smiles[slc], X[slc]):
                    img = Draw.MolsToGridImage(
                        [Chem.MolFromSmiles(smile)], molsPerRow=1, subImgSize=(200, 200)
                    )
                    imagebox = offsetbox.AnnotationBbox(
                        offsetbox.OffsetImage(img), (x, y), bboxprops=dict(color=color)
                    )
                    ax.add_artist(imagebox)
            else:
                # Plots points
                plt.scatter(
                    X[slc, 0], X[slc, 1], s=150 * size, color=color, label=label
                )
