...
class Args(Tap):
    ...
    split_type: Literal[
        "random", "scaffold", "time_window"
    ]  # Random or scaffold based split
    ...
    time_folds_per_train_set: int = (
        3  # X:1:1 train:val:test for time split sliding window
    )
    smiles_columns: List[str] = None  # columns in CSV dataset file containing SMILES
    split_key_molecule: int = (
        0  # index of the molecule to use for splitting in muli-molecule data
    )
...
def create_time_splits(args: Args):
    ...
    for i in range(args.num_folds - args.time_folds_per_train_set - 1):
        ...
        fold_indices["random"].append(
            split_indices(deepcopy(subset_indices), args.time_folds_per_train_set + 2)
        )
        ...
        fold_indices["time"].append(
            split_indices(
                subset_indices, args.time_folds_per_train_set + 2, shuffle=False
            )
        )
    for split_type in ["random", "scaffold", "time"]:
        ...
        for i in range(len(fold_indices[split_type])):
            os.makedirs(
                os.path.join(args.save_dir, split_type, "fold_" + str(i), "0"),
                exist_ok=True,
            )
            with open(
                os.path.join(
        ...
        with open(
            os.path.join(
                args.save_dir, split_type, "fold_" + str(i), "split_indices.pckl"
            ),
            "wb",
        ) as wf:
...
def create_crossval_splits(args: Args):
    ...
    for i in range(args.test_folds_to_test):
        ...
        for j in range(1, args.val_folds_per_test + 1):
            os.makedirs(
                os.path.join(args.save_dir, args.split_type, f"fold_{i}", f"{j - 1}"),
                exist_ok=True,
            )
            with open(
                os.path.join(
        ...
        with open(
            os.path.join(
                args.save_dir, args.split_type, f"fold_{i}", "split_indices.pckl"
            ),
            "wb",
        ) as wf:
            pickle.dump(all_splits, wf)
