...
def load_preds_and_targets(
    preds_dir: str, experiment: str, dataset: str, split_type: str
) -> Tuple[Optional[np.ndarray], Optional[np.ndarray]]:
    all_preds, all_targets = [], []
    num_folds = 0
    for fold in range(10):
        preds_path = os.path.join(
            preds_dir, f"417_{experiment}", dataset, split_type, str(fold), "preds.npy"
        )
    ...
    if num_folds not in [3, 10]:
        print(
            f'Did not find 3 or 10 preds/targets files for experiment "{experiment}" and dataset "{dataset}" and split type "{split_type}"'
        )
        return None, None

    all_preds, all_targets = np.concatenate(all_preds, axis=0), np.concatenate(
        all_targets, axis=0
    )
...
def compute_values(
    dataset: str, preds: List[List[List[float]]], targets: List[List[List[float]]]
) -> List[float]:
...
def wilcoxon_significance(preds_dir: str, split_type: str):
    print(
        "dataset\t" + "\t".join([f"{exp_1} vs {exp_2}" for exp_1, exp_2 in COMPARISONS])
    )
    ...
    for dataset in DATASETS:
        ...
        for experiment in EXPERIMENTS:
            ...
            preds, targets = load_preds_and_targets(
                preds_dir, experiment, dataset, split_type
            )  # num_molecules x num_targets
        ...
        for experiment_1, experiment_2 in COMPARISONS:
            if (
                "compare_lsc_scaffold" in [experiment_1, experiment_2]
                and split_type != "scaffold"
            ):
                continue

            values_1, values_2 = (
                experiment_to_values[experiment_1],
                experiment_to_values[experiment_2],
            )
            ...
            # Remove nans
            values_1, values_2 = zip(
                *[
                    (v_1, v_2)
                    for v_1, v_2 in zip(values_1, values_2)
                    if not (np.isnan(v_1) or np.isnan(v_2))
                ]
            )
