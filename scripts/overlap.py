...
class Args(Tap):
    ...
    smiles_columns_1: List[
        str
    ] = None  # Name of the column containing SMILES strings for the first data. By default, uses the first column.
    smiles_columns_2: List[
        str
    ] = None  # Name of the column containing SMILES strings for the second data. By default, uses the first column.
    save_intersection_path: str = (
        None  # Path to save intersection at; labeled with data_path 1 header
    )
...
def overlap(args: Args):
    ...
    smiles_1, smiles_2 = set([tuple(smiles) for smiles in smiles_1]), set(
    ...
    if args.save_intersection_path is not None:
        with open(args.data_path_1, "r") as rf, open(
            args.save_intersection_path, "w"
        ) as wf:
