...
class ChempropTests(TestCase):
    ...
    def interpret(
        self, dataset_type: str, checkpoint_dir: str, flags: List[str] = None
    ):
        # Set up command line arguments
    ...
    def test_train_single_task_regression(
        ...
        with TemporaryDirectory() as save_dir:
            ...
            self.assertAlmostEqual(
                mean_score, expected_score, delta=DELTA * expected_score
            )
    ...
    def test_predict_single_task_regression(
        ...
        with TemporaryDirectory() as save_dir:
            ...
            true = pd.read_csv(
                os.path.join(TEST_DATA_DIR, f"{dataset_type}_test_true.csv")
            )
    ...
    def test_multimolecule_fingerprint_with_single_input(
        ...
        with TemporaryDirectory() as save_dir:
            ...
            if exception_thrown:
                ...
                self.assertEqual(
                    list(fingerprints["smiles"].values),
                    list(test_input["smiles"].values),
                )
    ...
    def test_uncertainty_regression(
        ...
        with TemporaryDirectory() as save_dir:
            ...
            predict_flags.extend(
                ["--evaluation_scores_path", eval_path, "--test_path", test_path]
            )
    ...
    def test_uncertainty_class(
        ...
        with TemporaryDirectory() as save_dir:
            ...
            evaluation_scores_data = pd.read_csv(eval_path)
            self.assertAlmostEqual(
                evaluation_scores_data["synergy"][0],
                expected_score,
                delta=expected_score * DELTA,
            )
